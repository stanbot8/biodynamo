...
// -----------------------------------------------------------------------------
//
// Reproduces the OHSU 2026 "cytoplasmic tradewind" scenario in miniature
// (Nature Commun. s41467-026-70688-6):
//
//   - Point source at the back of a cell-sized cube (rear wall seeds mass)
//   - Uniform +x advection velocity throughout the interior (the "wind")
//   - Actin-myosin condensate barrier: a sealed plane near the front, with
//     a small central opening. Pure diffusion would leak mass uniformly;
//     advection forces mass through the gap and concentrates it at the
//     leading-edge compartment.
//
// Writes a 1D x-profile (y, z averaged) at t=0 and t=T_end to stdout in CSV,
// plus the mass in the leading-edge compartment (right of the barrier) over
// time. Against the pure-diffusion baseline (run with --no-wind), the
// advection run should show order-of-magnitude higher leading-edge mass.
...
int Simulate(int argc, const char** argv) {
  ...
  SeedRearSource(g);
  if (with_wind) SetUniformWind(g, kWind);
  InstallBarrier(g);

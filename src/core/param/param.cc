...
void AssignThreadSafetyMechanism(const toml::table& config, Param* param) {
  auto value = config.at_path("simulation.thread_safety_mechanism")
                   .value<std::string>();
  if (!value) {
    return;
  }
...
void Param::AssignFromConfig(const toml::table& config) {
  ...
  if (auto visualize_agentstarr = config["visualize_agent"].as_array()) {
    for (auto& elem : *visualize_agentstarr) {
      ...
      if (auto dm_arr =
              (*table)["additional_data_members"].as_array()) {
        for (auto& dm : *dm_arr) {
  ...
  //   visualize_diffusion
  if (auto visualize_diffusiontarr =
          config["visualize_diffusion"].as_array()) {
  ...
  if (auto sim_tbl = config["simulation"].as_table()) {
    if (auto ops_arr =
            (*sim_tbl)["unschedule_default_operations"].as_array()) {

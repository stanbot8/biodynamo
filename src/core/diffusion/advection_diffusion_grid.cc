...
real_t AdvectionDiffusionGrid::GetPermeability(size_t idx, int axis) const {
  ...
  if (axis == 0) return px_[idx];
  if (axis == 1) return py_[idx];
  if (axis == 2) return pz_[idx];
  return 1;
}

namespace {

/// First-order upwind flux at the face between `lo` and `hi` along an axis.
/// Positive `v_face` flows from lo -> hi. Returns the amount transported
/// per unit time per unit face area: v_face * c_upwind.
...
/// Full advection-diffusion step with closed (no-flux) edges.
///
/// Face permeability multiplies BOTH the diffusive and advective flux,
/// so sealed barriers (p = 0) fully isolate the two sides.
///
/// Velocity at a face is the average of the two voxel-centered velocities.
void AdvectionDiffusionGrid::DiffuseWithClosedEdge(real_t dt) {
  ...
  // Combined CFL: first-order upwind advection + FTCS diffusion.
  //   dt * (6 D / dx^2 + |v|_max / dx) < 1
  // Check once per run; the max velocity can grow later but the user should
  // see this early if their initial field is already unstable.
  if (!cfl_warned_) {
    real_t vmax = 0;
    ...
    if (cfl >= 1) {
      Log::Warning("AdvectionDiffusionGrid",
                   ...
                   " >= 1. Scheme is unstable. Reduce dt, D, or |v|, or ",
                   "increase resolution. Substance: '", GetContinuumName(),
                   "' dt=", dt, " D=", d, " dx=", dx, " |v|_max=", vmax);
      cfl_warned_ = true;
    }
  }
  ...
  for (size_t z = 0; z < nz; z++) {
    for (size_t y = 0; y < ny; y++) {
      for (size_t x = 0; x < nx; x++) {
        ...
        // Diffusive + advective flux accumulator.
        real_t flux_in = 0;

        // dc/dt = -div(v c) means inflow through a face adds mass to c.
        // At the -x face between voxels l and c, positive v_face transports
        // mass l -> c (inflow to c), so the sign is +v*c_upwind/dx.
        // At the +x face between c and r, positive v_face transports c -> r
        // (outflow from c), so the sign is -v*c_upwind/dx.
        // -x face
        if (x > 0) {
          const size_t l = c - 1;
...
// For now, other boundary conditions delegate to the closed-edge variant.
// Full Dirichlet/Neumann/periodic/open support with advection is a follow-up:
// the interaction between inflow boundaries and upwind stencils needs careful
// handling, and the closed-edge version covers the cytoplasmic-interior case
// the paper describes (the cell membrane is itself a no-flux boundary).
